...
terra_tag_descriptions = {
    ...
    "devices": "(?)",
    "device_values": "(?)",
    "version": "A version number. Probably of the TERRA software (?)",
...
def search_tag(tif, tag_id):
    ...
    """
    try:  # For older versions of tifffile, TiffFile objects are iterable and pages can be addressed directly.
        for page in tif:
...
def peem_dld_read_terra_sumimage(filepath):
    """
    Reads a tif file as generated by Terra when using the DLD. Therefore, the 3D tif dimensions are interpreted as
    time-channel, x and y. This method only reads the time channel `0`, which contains the sum image and returns a
    2D DataSet.
...
def peem_camera_read(filepath, mode="camware"):
    """
    Reads a PEEM image dataset acquired with the camera. Shadows the different readin functions for the different
...
def peem_camera_read_terra(filepath):
    """
    Reads a PEEM image acquired with the Camera and saved with Terra. For now, this just shadows the
...
def powerlaw_folder_peem_dld(folderpath, pattern="mW", powerunit=None, powerunitlabel=None, h5target=False,
    ...
    if norm_to_exptime:
        # Defining regex for getting acquisition time
        tunitm = "m"
        tunits = "s"
        tunith = "h"
    ...
    for filename in filter(is_tif, os.listdir(folderpath)):
        ...
        if foundp:
            ...
            if norm_to_exptime:
                # calculate acquisition time in seconds and add it to powerfiles list as norming factor
                ...
                if exp_time == 0:
                    print('WARNING in norm_to_exptime. Exposure time 0 or no exposure time detected for power ' + str(
                        power))
...
def tr_folder_peem_camera_terra(folderpath, delayunit="um", delayfactor=0.2, delayunitlabel=None,
    ...
    """
    if len(kwargs):
        warnings.warn("Unrecognized (probably depreciated) keyword args used in tr_folder_peem_dld_terra!",
                      DeprecationWarning)
