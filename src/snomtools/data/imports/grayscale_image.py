...
def timelog_folder(folderpath, timeunit='s', timeunitlabel=None,
    ...
    for filename in filter(is_image_file, os.listdir(folderpath)):
        ...
        if timestring.startswith(prefix):
            ...
            continue
        if timestring.endswith(postfix):
    ...
    if h5target:
        ...
        dataarray = ds.DataArray(dataspace,
                                 ...
                                 chunk_cache_mem_size=use_cache_size)
        dataset = ds.DataSet("Time Series " + folderpath, [dataarray], axlist, h5target=h5target,
                             chunk_cache_mem_size=use_cache_size)
    else:
...
def image_series(folderpath, numbered=False,
                 prefix="", postfix="",
                 h5target=True):
    """
    ...
    :param prefix: Leading part of the filenames.
            Files that do not contain the prefix will be ignored.

    :param postfix: Trailing part of the filenames, without extension.
            Files that do not contain the postfix will be ignored.

    :param h5target: optional, set a h5 group or path on which the data is being saved on (NOT SURE)

    :return: The dataset containing the images stacked along a file_number axis.
    ...
    for filename in filter(is_image_file, sorted(os.listdir(folderpath))):
        ...
        if numbered:
            numbered_files[int(timestring.strip())] = filename
        else:
            numbered_files[i] = filename
            i += 1

    # Build file_number axis:
    axlist = []
    for file_number in iter(sorted(numbered_files.keys())):
        axlist.append(file_number)
    pl = 'File #' # Plot label for file_number axis.
    file_number_axis = ds.Axis(axlist, label='file_number', plotlabel=pl)

    # ----------------------Create dataset------------------------
    # Test data size:
    sample_data = read_image(os.path.join(folderpath, numbered_files[list(numbered_files.keys())[0]]))
    axlist = [file_number_axis] + sample_data.axes
    newshape = file_number_axis.shape + sample_data.shape
    # Build the data-structure that the loaded data gets filled into
    if h5target:
        chunks = True
        compression = 'gzip'
        ...
        if chunks is True:  # Default is auto chunk alignment, so we need to probe.
            ...
            chunk_size = chunks
        use_cache_size = buffer_needed(newshape, (0,), chunk_size, dtype=sample_data.get_datafield(0).data.dtype)
        ...
        dataarray = ds.DataArray(dataspace,
                                 ...
                                 compression=compression, compression_opts=compression_opts,
                                 chunk_cache_mem_size=use_cache_size)
        dataset = ds.DataSet("Image Sequence " + folderpath, [dataarray], axlist, h5target=h5target,
                             chunk_cache_mem_size=use_cache_size)
    else:
        # In-memory data processing without h5 files.
        dataspace = u.to_ureg(np.zeros(newshape), sample_data.datafields[0].get_unit())
    ...
    if verbose:
        ...
        if h5target:
            ...
            print("... in memory")
        start_time = time.time()
    for i, file_number in zip(list(range(len(numbered_files))), iter(sorted(numbered_files.keys()))):
        islice = (i,) + slicebase
        # Import image:
        idata = read_image(os.path.join(folderpath, numbered_files[file_number]))
